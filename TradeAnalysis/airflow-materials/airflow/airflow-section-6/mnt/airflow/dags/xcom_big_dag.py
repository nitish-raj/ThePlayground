...
def generate_random_dates(start, end, n):
    ...
    for i in range(n):
        dates[i] = start + datetime.timedelta(
...
def push_xcom_with_return():
    ...
    df = pd.DataFrame(
        {
            "user_id": sample(range(90000000, 99999999), n),
            "order_id": np.random.choice(range(1000000, 2000000), n, replace=False),
            "order_date": generate_random_dates(
                datetime.date(2015, 1, 1), datetime.date(2017, 12, 31), n
            ),
    ...
    df = df.assign(
        day_of_week=df.order_date.apply(lambda x: calendar.day_name[x.weekday()])
    )
...
with DAG(dag_id="xcom_dag_big", default_args=args, schedule_interval="@once") as dag:
    ...
    t0 = PythonOperator(task_id="t0", python_callable=push_xcom_with_return)

    t1 = PythonOperator(
        task_id="t1", provide_context=True, python_callable=get_pushed_xcom_with_return
    )
