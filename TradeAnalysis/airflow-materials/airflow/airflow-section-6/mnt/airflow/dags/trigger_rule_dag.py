...
with DAG(
    dag_id="trigger_rule_dag", default_args=default_args, schedule_interval="@daily"
) as dag:
    ...
    process_task = PythonOperator(
        task_id="process", python_callable=process, trigger_rule="all_success"
    )

    notif_a_task = PythonOperator(
        task_id="notif_a", python_callable=notif_a, trigger_rule="all_success"
    )

    notif_b_task = PythonOperator(
        task_id="notif_b", python_callable=notif_b, trigger_rule="all_success"
    )
